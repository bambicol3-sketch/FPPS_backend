"""나딱구 공부방 신용 스프레드 분석 페이지를 1회 크롤링하여 DB 에 스냅샷으로 저장한다.
...
from app.domains.mse_credit.adapter.outbound.external.mse_credit_crawler import (  # noqa: E402
    CreditCrawler,
)
from app.domains.mse_credit.adapter.outbound.persistence.credit_snapshot_repository_impl import (  # noqa: E402
    CreditSnapshotRepositoryImpl,
)
from app.domains.mse_credit.application.usecase.crawl_credit_usecase import (  # noqa: E402
    CrawlCreditUseCase,
)
import app.domains.mse_credit.infrastructure.orm.credit_snapshot_orm  # noqa: E402, F401
from app.infrastructure.database.database import AsyncSessionLocal, Base, engine  # noqa: E402


DEFAULT_PAGE_URL = "https://jamesgong.duckdns.org/stock/credit-spread/"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Crawl mse_credit page once")
    parser.add_argument("--url", default=DEFAULT_PAGE_URL, help="Credit spread page URL")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.cookie:
        ...
        username = args.username or input("mse_credit username: ").strip()
        password = args.password or getpass.getpass("mse_credit password: ")
    ...
    async with AsyncSessionLocal() as db:
        ...
        snapshot = await CrawlCreditUseCase(crawler, repository).execute()
