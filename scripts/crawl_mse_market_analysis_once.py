"""나딱구 공부방 시장분석 페이지를 1회 크롤링하여 DB 에 스냅샷으로 저장한다.

자격 증명은 영구 보관하지 않으며 실행 시점에만 사용된다.
사용법은 scripts/crawl_study_room_once.py 와 동일.
"""
...
from app.domains.mse_market_analysis.adapter.outbound.external.mse_market_analysis_crawler import (  # noqa: E402
    MarketAnalysisCrawler,
)
from app.domains.mse_market_analysis.adapter.outbound.persistence.market_analysis_snapshot_repository_impl import (  # noqa: E402
    MarketAnalysisSnapshotRepositoryImpl,
)
from app.domains.mse_market_analysis.application.usecase.crawl_market_analysis_usecase import (  # noqa: E402
    CrawlMarketAnalysisUseCase,
)
import app.domains.mse_market_analysis.infrastructure.orm.market_analysis_snapshot_orm  # noqa: E402, F401
from app.infrastructure.database.database import AsyncSessionLocal, Base, engine  # noqa: E402


DEFAULT_PAGE_URL = "https://jamesgong.duckdns.org/stock/market/"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Crawl mse_market_analysis page once")
    parser.add_argument("--url", default=DEFAULT_PAGE_URL, help="Market analysis URL")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.cookie:
        ...
        username = args.username or input("mse_market_analysis username: ").strip()
        password = args.password or getpass.getpass("mse_market_analysis password: ")
    ...
    async with AsyncSessionLocal() as db:
        ...
        snapshot = await CrawlMarketAnalysisUseCase(crawler, repository).execute()
